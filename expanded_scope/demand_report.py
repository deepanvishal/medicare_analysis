...
def build_overview(wb, d):
    ...
    for lab, val in [
        ...
        ("HCC labels", "HCC category names = standard CMS-HCC V24 model (built into the report; verify vs the CMS V24 file)."),
    ]:
...
# ---------- CMS-HCC V24 category names (standard model; verify vs the CMS V24 file) ----------
HCC_V24_LABELS = {
    "1": "HIV/AIDS",
    ...
    "56": "Drug/Alcohol Abuse, Uncomplicated, Except Cannabis",
    "57": "Schizophrenia",
    ...
    "74": "Cerebral Palsy",
    "75": "Myasthenia Gravis/Myoneural Disorders and Guillain-Barre Syndrome",
...
def _hcc_key(v):
    """Normalize an HCC code to a bare string key ('115.0'/115 -> '115')."""
    try:
...
def build_hcc(wb, hcc):
    out = hcc.copy()
    out["hcc_code"] = out["hcc_code"].map(_hcc_key)
    out["hcc_description"] = out["hcc_code"].map(HCC_V24_LABELS).fillna("(label not in V24 map -- verify)")
    out = out.sort_values(["state_cd", "utilization"], ascending=[True, False])
    cols = [
        ("state_cd", "State", 8, None, "left"),
        ("hcc_code", "HCC v24", 9, None, "center"),
        ("hcc_description", "HCC Category (CMS-HCC V24)", 46, None, "left"),
        ("payer", "Payer", 11, None, "center"),
    ...
    simple_table(wb, "4. HCC Morbidity", out, cols,
                 ...
                 "category names = standard CMS-HCC V24 model.",
                 payer_key="payer")
