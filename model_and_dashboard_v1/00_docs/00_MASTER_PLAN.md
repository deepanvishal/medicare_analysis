...
| 21 | 06_outputs | 21_dashboard_extracts.py | EXECUTED SPEC. Writes the eight parquet extracts + manifest.json to 07_dashboard/extracts/: enrollment (with county_name), growth_context (label only, defaults 0 per D11), sickness_rates, visit_rates (with deflation_factor), county_calibration, providers (capacity v0 per D14), conditions_meta, other_condition_prevalence | outputs of 06, 07, 08, 11 (context), 14, 16 v0, A2 tables | No empty extract (R1); enrollment counties match 2025-12 exactly (R4) and all named (R7); other-prevalence 0..1 + key unique (R2); manifest carries the capacity=v0 line |
...
| 23 | 07_dashboard | whatif_dashboard.py + whatif_dashboard_v2.py | Deliverable 2, WIRED (MVP); v2 adds tabs, state/county-name filters, the OTHER_CONDITION term (exact baseline), and the methodology/model-detail pages on port 8051 | outputs of 21 | Dashboard math matches 19 calibration |
